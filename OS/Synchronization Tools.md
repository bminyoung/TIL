...
- 임계구역 문제 해결을 위한 요구 조건
  ...
  2. **진행**: 임계구역에서 실행되는 프로세스가 없고, 진입하려고 하는 프로세스가 있는 경우
     ...
     => deadlock(임계구역에 아무도 못들어가는 상황)이 생기면 안됨

  3. **한정된 대기**: 프로세스가 임계구역 진입 요청을 한 후 허용될 때까지, 다른 프로세스가 임계구역에 진입하도록 허용되는 횟수에 한계가 있어야 함

     => 기아현상(진입 요청 후 다른 프로세스에게 밀리고 밀려 무한 대기)이 발생하면 안됨
...
- Mutex 락 유형을 **스핀락** 이라고 한다. (락이 가용 상태가 될 때까지 프로세스가 회전하기 때문)
  - 싱글 코어에서는 문제가 될 수 있음 (CPU 낭비)
  - 멀티 코어에서는 context switch 시간을 아낄 수 있음
...
> 프로세스가 실행 수명주기동안 진행되는 것을 보장하기 위해 충족해야하는 속성

- `무한 루프`는 라이브니스의 실패 예시 중 하나



#### 1) 교착 상태 (Deadlock)

- 대기 중인 프로세스가 발생시키는 이벤트를 무한정 기다리는 경우

- 프로세스1이 wait(P) - signal(S) 실행

  프로세스2가 wait(S) - signal(P) 실행

  => 프로세스1의 wait(P) 실행을 위해 프로세스2의 signal(P)를 실행해야함,

  -> wait(S)를 실행해야함 -> 프로세스1의 signal(S)를 실행해야함 -> wait(P)를 실행해야함 -> ...
...
- 커널 데이터는 락에 의해 보호된다.
  ...
  => 낮은 우선순위의 프로세스가 사용 중인 자원을 높은 우선순위의 프로세스가 기다려야 함

- 우선순위가 L < M < H 인 3개의 프로세스에서 L이 사용 중인 자원을 H가 기다리는 상황

  -> M이 실행 가능 상태가 되어 L을 선점

  -> H는 M의 실행 시간만큼을 더 기다려야 함

  **=> 우선순위 역전**

- `우선순위 상속 프로토콜` 로 해결 가능
  - 위의 경우 H가 필요로 하는 자원을 사용 중인 L은 임시로 H의 우선순위를 상속받음