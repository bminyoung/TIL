...
- 연성 실시간 시스템: 실시간 프로세스가 스케줄되는 시점에 대한 보장 x

  (중요 프로세스의 우선권만 보장)

- 경성 실시간 시스템: 엄격한 요구 조건을 만족시켜야 함



#### 1) 지연시간 최소화

- 이벤트 지연시간: 이벤트 발생 후 그에 맞는 서비스가 수행될 때까지의 시간

- 실시간 시스템의 성능을 좌우하는 2가지 유형: 인터럽트 지연시간, 디스패치 지연시간

  1. 인터럽트 지연시간: (인터럽트 발생~인터럽트 처리 루틴 시작)까지의 시간

     = (인터럽트 발생 -> 인터럽트의 종류 결정 -> 수행 중인 프로세스 상태 저장) 까지의 시간

  2. 디스패치 지연시간: 디스패처가 하나의 프로세스를 블록, 다른 프로세스를 시작하기까지의 시간

     = (충돌 단계 + 디스패치 단계)동안의 시간

     ※ 충돌 단계: [커널에서 동작하는 프로세스에 대한 선점]

     ​				 + [높은 우선순위 프로세스에서 필요한 자원을 낮은 우선순위 프로세스에서 방출]



#### 2) 우선순위 기반 스케줄링

- 실시간 운영체제에서는 **실시간 프로세스에 CPU가 필요할 때 바로 응답해주는 것**이 가장 중요
- 연성 실시간 기능만을 제공

- 우선순위를 정하는 요소: 수행 시간(t), 마감시간(d), 주기(p) (0 <= t <= d <= p)



#### 3) Rate-Monotonic 스케줄링

> 높은 우선순위의 프로세스가 ready상태가 되며 낮은 우선순위의 프로세스를 선점

- CPU사용 주기가 짧은 프로세스에 높은 우선순위 부여
- 정해진 우선순위는 변하지 않음 (정적 우선순위)
- 정적 우선순위 스케줄링 중 최적
...
- 프로세스 실행 시 마감시간을 시스템에 알림 -> 우선순위 재조정
- 동적 우선순위 스케줄링 중 최적
...
> 전체 시간 몫(T) 대비 할당된 시간 몫(N)만큼을 프로세스에 할당

- 전체=200, A에 100, B에 30, C에 40을 할당하면

  => A는 50%, B는 15%, C는 20% 할당받는 것

  (+) 새로운 프로세스 D 가 40의 몫을 요구하면 시스템 진입 거부 (100(A)+30(B)+40(C)+40(D) > 200)
...
- 분석적 평가: 알고리즘의 성능을 평가하는 공식, 값 생성을 위해 알고리즘과 시스템 작업 부하 이용

- 결정론적 모델링: 분석적 평가의 한가지 유형

  - 특정 작업 부하를 받아들여 그 작업 부하에 대한 알고리즘의 성능 정의
...
- | 프로세스 | 버스트 시간 |
  ...
  표와 같은 상황에서 FCFS, SJF, RR 알고리즘을 고려 => 평균 대기 시간 계산
  
  1. FCFS: 28
  2. SJF: 13
  3. RR: 23
  
- 단순하고 빠르며 비교가능한 정확한 값 제공

- 주로 알고리즘 설명, 예시를 제공하는 데 사용
